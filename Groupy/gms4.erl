...
slave(Id, Master, Leader, N, Last, Slaves, Group)->
    receive
        ...
        {msg, N, Msg}->
            Master ! Msg,
            %Send the acknologement to the Leader.
            Leader ! ack,
            %The last message seen from Leader is {msg, N, Msg}.
...
bcast_ack(Id, Msg, Nodes) ->
    %io:format("Leader ~p broadcasting ~p to ~p~n", [Id, Msg, Nodes]),
    lists:foreach(fun(Node)-> randskip(Node,Msg), ack(Node,Msg) ,crash(Id) end, Nodes).

ack(Node,Msg)->
    receive
        ack->
            ok
    after 1000->%resend if time is out
        io:format("Resending Msg to Node~p~n",[Node]),
        Node ! Msg,
        ack(Node,Msg)
    end.
...
randskip(Node,Msg)->
    case rand:uniform(200) of 
        200->
            io:format("Broadcast skips Node ~p~n",[Node]);
        _->
            Node ! Msg
    end.
